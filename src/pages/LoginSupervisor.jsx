...
function FormLogin({ onLogin, onNuevaCuenta, onVolver, onRecuperar }) {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (e.message === 'NO_ENCONTRADO') setError('No encontramos tu cuenta. ¿Ya te registraste? Si no, crea tu cuenta.')
  ...
  return (
    <div className="min-h-screen bg-brand-black flex flex-col items-center justify-center px-6 py-10">
      <div className="mb-8 text-center">
        <div className="w-16 h-16 rounded-2xl bg-purple-600 flex items-center justify-center text-3xl mx-auto mb-3">📊</div>
      ...
      <div className="w-full max-w-sm space-y-4">
        ...
        <div className="space-y-3">
          <div>
            ...
            <select value={nombre} onChange={e => setNombre(e.target.value)}
              ...
              {SUPERVISORES_LISTA.map(n => <option key={n} value={n}>{n}</option>)}
            </select>
          </div>
          <div>
        ...
        <div className="text-center">
          <button onClick={onRecuperar} className="text-sm text-gray-500 hover:text-purple-400 transition-all underline underline-offset-2">
...
function RecuperarPin({ onVolver }) {
  ...
  async function handleEnviarCodigo() {
    ...
    setLoading(true)
    try {
      const data = await buscarSupervisorParaRecuperacion(nombre)
      ...
      await guardarCodigoRecuperacionSupervisor(data.id, codigo)
  ...
  async function handleGuardarPin() {
    ...
    setLoading(true)
    try {
      await actualizarPinSupervisor(supId, nuevoPin)
      setPaso('listo')
  ...
  return (
    <div className="min-h-screen bg-brand-black flex flex-col items-center justify-center px-6 py-10">
      <div className="mb-6 text-center">
        ...
        <p className="text-xs text-gray-500">Supervisor · Academia Guerrera</p>
      </div>
        ...
        <div className="flex gap-1.5">
          {['buscar','codigo','nuevo_pin'].map((p, i) => (
            <div key={p} className={`flex-1 h-1.5 rounded-full transition-all ${
              ['buscar','codigo','nuevo_pin','listo'].indexOf(paso) > i ? 'bg-purple-500' : paso === p ? 'bg-purple-500/60' : 'bg-brand-medium'
            }`} />
          ))}
        </div>
      </div>
      ...
      <div className="w-full max-w-sm space-y-4">
        ...
        {paso === 'codigo' && (
          <>
            <div>
              ...
              <p className="text-sm text-gray-500">Enviamos un código de 6 dígitos a <span className="text-purple-400 font-bold">{correoInput}</span></p>
        ...
        {paso === 'listo' && (
          <div className="text-center space-y-4">
            ...
            <button onClick={onVolver} className="w-full bg-purple-600 text-white font-black py-4 rounded-2xl text-base hover:bg-purple-500 transition-all">
              Ir a iniciar sesión →
            </button>
          </div>
        )}
